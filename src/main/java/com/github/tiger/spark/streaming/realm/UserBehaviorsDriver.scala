...
object UserBehaviorsDriver {
  ...
  def createContext(): StreamingContext = {
    ...
    val userBehaviors: DStream[(String, BehaviorState)] =
      messages.mapWithState[BehaviorState, (String, BehaviorState)](stateSpec)

    userBehaviors.print(200)
    ...
    userBehaviors.foreachRDD(rdd => {
      ...
      rdd.foreach(pair => {
        ...
        val transportAddresses = Array(
          ...
          new InetSocketTransportAddress(
            InetAddress.getByName("175.63.101.108"), 9300).asInstanceOf[TransportAddress],
          new InetSocketTransportAddress(
            InetAddress.getByName("175.63.101.109"), 9300).asInstanceOf[TransportAddress]
        )
  ...
  def main(args: Array[String]): Unit = {
    ...
    val _checkpoint = "hdfs://spark-220/user/zpcampus/user/behavior/job/checkpoint"
